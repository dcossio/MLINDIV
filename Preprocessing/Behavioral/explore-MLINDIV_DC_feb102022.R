# Explore script for prepossessing 
#edited for midlife/young on slow computer 
#feb3. DC abd VP
# putting all the code together so far to see where we're at. So far it does not combine verything yet 
...
library(wrapr)
library(plyr)
library(BRRR) #just a funsies library. can remove 

# DJ Khaled if theres an error
options(error = function() {skrrrahh(34)})

working_dir <- "/Users/danielacossio/Desktop/MLINDIV_SNAG_preprocessing/old"
setwd(working_dir)
...
for (participant_file_folder in 1:length(MLINDIV_filelist)){
  ...
  for (file_i in 1:length(eprime_txt_files)){
    ...
    # List of variable to pull from E prime files 
    ...
    #creating explore tibble and removing from data frame
    expl_df <-e_df[e_df$Procedure == "ExploreProc", ]
    expl_proc <- as.tibble(expl_df) %>%
      select(qc(
  ...
  #---------------------------------------------------------------------------------------------------------------------------------------------------------    
    ...
    trial_tibble <- as.tibble(Noexpl_df) %>%
      select(qc(
  ...
  #----------------------------------------------------------------------------------------------------------------------------------------------------------
    # Begin the explore processing 
     ...
     explr_tibble <- as.tibble(expl_df) %>%
      select(qc(
        Eprime.Basename,Eprime.LevelName,
        ImageFile,Choose.OnsetTime,
        Choose.RTTime, Choose.RT, 
   ...
   explr_tibble <- cbind(expl_proc, explr_tibble)
    ...
    Subject <- e_df_header$Subject
    #Task type may be un needed, Will delete in the future if that's the case
    Task_type <- strsplit(expl_proc$Cycle, "-")[[1]][1] # this is labeling everything as cycle 1 not 2 or 3 
    #Task_type <- paste0(strsplit(Task_type, "_")[[1]][5], strsplit(Task_type, "_")[[1]][6])#adds NANANA for whatever reason
    
    expl_meta_df <- cbind(Subject, Task, Task_type, Finished.OnsetTime)
...
write.csv(master_file, "MLINDIV_behavioral_master_2.csv")
